  ...
  return (
    <div className="container">
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <h3>Shop List</h3>
        <Link to="/add-shop" className="btn btn-primary mb-2 ">
          {" "}
          Add Shop{" "}
        </Link>
      </div>
          ...
          {loading ? (
            Shops.map((shop) => (
              <tr key={shop.shopId}>
                ...
                <td>{shop && shop.shopOwner ? shop.shopOwner["name"] : ""}</td>{" "}
                <td>
                  <Link
                    className="btn btn-outline-primary"
                    to={`/update-shop/${shop.shopId}`}
                  >
                    {" "}
                    UPDATE
                  </Link>
            ...
            <>
              ...
              <Spinner
                animation="grow"
                variant="success"
                style={{ position: "fixed", top: "50%", left: "56%" }}
              />
              <Spinner
                animation="grow"
                variant="danger"
                style={{ position: "fixed", top: "50%", left: "52%" }}
              />
              <Spinner
                animation="grow"
                variant="warning"
                style={{ position: "fixed", top: "50%", left: "54%" }}
              />
            </>
          )}
        </tbody>
